...
test('charge monitoring: isEnableBilling', async t => {
  ...
  sinon
    ...
    .once()
    .withArgs({ name: PROJECT_NAME, auth: null })
    .resolves({
      data: {
        billingEnabled: true,
      },
    });
  const result = await isEnableBilling(PROJECT_NAME, cloudbilling.projects, null);
  t.true(result);
});
...
test('charge monitoring: disableBilling', async t => {
  ...
  // run disable billing
  const result = await disableBilling(PROJECT_NAME, cloudbilling.projects, null);
